...
### This file builds the campus graph and solves routes using Dijkstra's algorithm
#Note: The distance and graph parts are new for this project.
#The interpolation idea is based off of the interpolation homework style from HW3.


MASTER_FILE = Path("data/master_map_data.csv")

#These are the extra assumptions for the route model
#I kept them together so they are easy to change for testing
...
#Function for converting longitude and latitude into web mercator x/y meters
#This makes the distance calculations work with the map points
...
#Function for converting mph to meters per second
def mph_to_mps(speed_mph):
    return speed_mph * 1609.344 / 3600


#Function for the distance between two xy points
...
#This is simpler than the Lagrange/Vandermonde interpolation from HW3,
#but it follows the same idea of estimating values between known data points.
...
#Function for estimating scooter speed from a segment length
#This is the interpolation based feature from the project proposal
#Short segments get a lower speed, and longer segments get closer to the normal scooter speed
...
#Function for loading the master csv file
...
#Function for grouping line features together
#This takes all the separate rows for one road/sidewalk and puts them in order
def group_line_features(data, feature_type):
    ...
    #loop through each feature and sort its points
...
#Function for adding an undirected edge
...
#Function for finding an existing node close to a point or making a new one
#This helps merge hand clicked points that are basically the same intersection
...
#Function for finding nearest node of a certain type
...
#Function for finding all nearby nodes of certain types
...
#Function for connecting close nodes of the same type after all hand drawn data is loaded
#This is needed because the sidewalk data was clicked by hand and some segments almost touch
...
#Function for building the full campus graph
#This is where the master csv gets turned into something Dijkstra can use
def build_campus_graph():
    ...
    #roads can be used by cars, bikes, and scooters
    ...
    #sidewalks can be used by walking, bikes, and scooters
    ...
    #this is how the routes actually start and end at real places
    #loop through buildings and POIs and add them as real places
    for row in data:
    ...
    #cars drive to parking, then the route switches to walking
    #loop through parking access points
    for row in data:
    ...
    #loop through each parking lot and hook it to roads/sidewalks
    ...
    #connect sidewalks to roads at intersections so pedestrians can cross
    #this also lets bikes and scooters transfer between roads and sidewalks
...
#Function for getting the cost of an edge for a mode
...
#This was written for this project, but follows the normal shortest path algorithm idea
...
#Function for getting path distance no matter whether route was optimized by time
...
#Function for combining route pieces
...
#Function for solving non-car modes
#Walking, biking, and scooter routes can be handled as one normal shortest path
def solve_simple_mode(graph, start_id, end_id, mode, optimize_by="time"):

    return dijkstra(graph, start_id, end_id, mode, optimize_by)


#Function for solving the car route
#This is split into walk -> drive -> walk because cars have to use parking lots
